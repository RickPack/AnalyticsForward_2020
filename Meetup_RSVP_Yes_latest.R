...
relative_day_graph_helper <- function(){
  ...
  # For example, if it's 2020 and 30 days remain until the event and
  # 2020 has 40 registrations, but 2018 had 50 on that day, 
  # 2020 will appear below 2018.
...
if (!github_file) {
  ...
  allAF_frm_rladies <- bind_rows(AF15_frm, AF16_frm, AF17_frm, AF18_frm, AF19_frm,
   ...
   allRTA_events_future_func <- function(meetupgrp_name){
      tryCatch(
     {
   ...
   allRTA_events_future <- allRTA_events_future_func(meetupgrp_name)
  ...
  # created was 2020-01-17 for A>F 2020 but first RSVP shows as 2020-01-31 in Meetup
  # so not using
  ...
  allAF_frm$dates_yes_otheryear <- as.Date(
    format(
      # 2020 used in below line because future year
...
#          label = "2020 registrations recently went flat",
#          size = 5)
...
p3 <-
  ...
  ggtitle(label = paste0("Research Triangle Analysts 'Analytics>Forward' as of\n",
                         Sys.Date(), "\nChart 3 of 7"),
          subtitle = str_glue(as.numeric(today_days_to_event), " days remaining")) +
...
p5 <-
  ...
  ggtitle(label = paste0("Research Triangle Analysts 'Analytics>Forward' as of\n",
                         Sys.Date(), "\nChart 5 of 7"),
          subtitle = str_glue("ONLY the week of the event",
...
p6 <- ggplot(allAF_frm_dt_day,
  ...
  ggtitle(label = str_glue("Registrations for Research Triangle Analysts 'Analytics>Forward'\n",
                           "March 9, 2020 at Blue Cross and Blue Shield NC (Durham)\n",
...
grp_plot <- hchart(grp_members2, "line",
  ...
  hc_subtitle(text = str_glue("Preparing for Analytics>Forward 2020: keynote by ",
                              "Dr. Robert Erhardt ", "(Climate Data)")) %>%
...
if (save_to_folder) {
  ggsave(p1, file = "af_2020-1.png", dpi = 72, width = 12, height = 5)
  ggsave(p2, file = "af_2020-2.png", dpi = 72, width = 12, height = 5)
  ggsave(p3, file = "af_2020-3.png", dpi = 72, width = 12, height = 5)
  ggsave(p4, file = "af_2020-4.png", dpi = 72, width = 12, height = 5)
  ggsave(p5, file = "af_2020-5.png", dpi = 72, width = 12, height = 5)
  ggsave(p6, file = "af_2020-6.png", dpi = 72, width = 12, height = 5)
  ggsave(p7, file = "af_2020-7.png", dpi = 72, width = 12, height = 5)
#  cowplot::save_plot("af_2020-1.png", p1)
#  Used Paint to change resolution to avoid truncation when animate used
  intro_image <- magick::image_read('Marketplace_Zillow_JordanMeyer_864x360.png', 
                                    strip = TRUE) 
  p1_image    <- image_read("af_2020-1.png") 
  p2_image    <- image_read("af_2020-2.png")
                 # image_annotate(str_glue("Will 2020 set a new record?"),
                 # size = 20, gravity = "northwest", color = "blue") %>% 
  p3_image    <- image_read("af_2020-3.png") 
  p4_image    <- image_read("af_2020-4.png") 
  p5_image    <- image_read("af_2020-5.png") 
  p6_image    <- image_read("af_2020-6.png") 
  p7_image    <- image_read("af_2020-7.png") 
